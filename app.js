...
socket.on('gatherInfo', function(view) {
  ...
  model.stockModel.find({}, function(err, dbResults) {
    ...
    // look up current stock info with yahoo finance
    for (var i = 0, l = dbResults.length; i < l; i++) {
      yahooFinance.quote({
      ...
      for (var key in result) {
        ...
        if (result[key].length >= 10) {
          for (var i = 0, l = result[key].length; i < l; i += Math.floor(l / 10)) {
            object.data.unshift(parseFloat(result[key][i].open.toFixed(2)));
            if (bool) {
              var d = new Date(result[key][i].date);
              info.dates.unshift(dateFormat(d, 'm/d/yy'));
            }
          }
...
socket.on('addStock', function(stock) {
  ...
  yahooFinance.quote({
    ...
    // stock doesn't exist
    if (err) return socket.emit('stockDoesntExist');

    // check if stock is already in database
    model.stockModel.findOne({
      symbol: stock
      ...
      if (dbResults === null) {
        ...
        var s = model.stockModel({symbol: stock});
        s.save(function(err) {
          if (err) throw err;
...
socket.on('removeStock', function(stock) {
  ...
  model.stockModel.findOneAndRemove({
    symbol: stock
  }, function(err) {
    if (err)throw err;
